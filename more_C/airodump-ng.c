...
/* Function definitions for rocks_DB */
void open_rocksDB();
void close_rocksDB();
int check_val_rocksDB(char*);

/* Variable Definitions for rocks_DB */
rocksdb_t *db;
...
/* The wifi packet specs tell us that power management
 * is in the fourth bit of the second byte in the
 * frame control header of the wifi packet
 */
#define POWER_MGMT_BIT_POS (4)
#define DS_BIT_POS (0)
#define TS_BIT_POS (1)
#define POWER_MGMT_BIT (1<<POWER_MGMT_BIT_POS)
#define DS_BIT (1<<DS_BIT_POS)
#define TS_BIT (1<<TS_BIT_POS)

/* Encryption function declaration */
void sha_encryption(char*,unsigned char*,char*);

/* Livereach Media constant definitions */

/* From the adxprism database on the LivereachMedia database server,
 * we checked the size of the deviceid column in the devices table.
 * The size of this column is 21, therefore the maximum possible
 * length of a device_id is 21. */
#define MAXIMUM_DEVICE_ID_LENGTH    21
/* Special device id to send when unable to determine device id from
 * the device */
#define DEVICE_ID_NOT_FOUND         "111111111111111111111"
/* This value is arbitrary */
#define MAXIMUM_TIMESTAMP_LENGTH    21
#define MAXIMUM_BUFFER_LENGTH       80
...
void open_rocksDB()
{
    ...
    rocksdb_options_set_compression_options(options,4,5,6,7);
    char *err = NULL;
    db = rocksdb_open(options, "/usr/sbin/db", &err);
}
...
int check_rocksDB(char* key)
{
    readoptions = rocksdb_readoptions_create();
    char* err = NULL;
    char* val=rocksdb_get(db,readoptions,key,strlen(key),&size,&err); 
...
int dump_initialize( char *prefix, int ivs_only )
{
    ...
    /* create the output protobuf file */
    if (G.output_format_protobuf) {
        memset(ofn, 0, ofn_len);
        snprintf(ofn, ofn_len, "%s-%02d.%s", prefix, G.f_index, AIRODUMP_NG_PROTOBUF_EXT);

        if ((G.f_protobuf = fopen(ofn, "wb+")) == NULL) {
            perror("fopen failed");
            fprintf(stderr, "Could not create \"%s\".\n", ofn);
            free(ofn);
            return (1);
...
struct packet_info* create_packet_info_node()
{
    struct packet_info *packet_info_node = (struct packet_info* ) malloc(sizeof(struct packet_info));
    memset(packet_info_node, 0, (sizeof(struct packet_info)));
    if (!packet_info_node)
        return NULL;
    packet_info_node->next = NULL;
    return packet_info_node;
}
// Inserting the node from end.
struct packet_info* insert_packet_info_node(struct packet_info *packet_info_node)
{
    struct packet_info *new_node = create_packet_info_node();
    new_node->next = packet_info_node;
    if (new_node == NULL)
        return NULL;
    return new_node;
}
...
{
    fprintf(stdout,"In dump_add_packet\n");
    
    int i, n, seq, msd, dlen, offset, clen, o,loopCntr;
    unsigned z;
    int loopcounter = 0;
    ...
    char mac_des_length[8] = {0};
    unsigned char *final_mac_des_length = NULL;
    ...
    /* printing the h80211 array for debugging */
    /*int iter=0;
    for( iter=0; iter<strlen(h80211); iter++ )
    {
        fprintf(stdout,"%02x ",h80211[iter]);
    }*/
    ...
    if( ap_cur == NULL )
    {
        ...
        /* update the power bit in frame control packet */
        ap_cur->power_bit = 0;
        /* Grab the values only if we have no transmitters */
        if( ( h80211[1] & 3 ) != 3) {
            ap_cur->power_bit = (h80211[1] & POWER_MGMT_BIT) &
                                (h80211[1] & DS_BIT) &
                                  (h80211[1] & TS_BIT);
        }
...
//         if(ap_cur->fcapt >= QLT_COUNT) update_rx_quality();
    ...
    switch( h80211[1] & 3 )
    {
        case  0:
            ...
            if( memcmp( h80211 + 10, bssid, 6 ) == 0 )
                ...
                //fprintf(stdout,"h80211: %02x:%02x:%02x:%02x:%02x:%02x\n",h80211[10],h80211[11],h80211[12],h80211[13],h80211[14],h80211[15]);
                goto skip_station;

            memcpy( stmac, h80211 + 10, 6 );
            //fprintf(stdout,"STMAC case 0: %02x:%02x:%02x:%02x:%02x:%02x\n",stmac[0],stmac[1],stmac[2],stmac[3],stmac[4],stmac[5]);
            break;
        ...
        case  1:
            ...
            memcpy( stmac, h80211 + 10, 6 );
            //fprintf(stdout,"STMAC case 1: %02x:%02x:%02x:%02x:%02x:%02x\n",stmac[0],stmac[1],stmac[2],stmac[3],stmac[4],stmac[5]);
            break;
        ...
        case  2:
            ...
            //fprintf(stdout,"STMAC case 2: %02x:%02x:%02x:%02x:%02x:%02x\n",stmac[0],stmac[1],stmac[2],stmac[3],stmac[4],stmac[5]);
            break;
    ...
    char idString[ID_STRING_LEN] = { 0 };
    unsigned char outputStr[SHA512_DIGEST_LENGTH] = { 0 };
    char* finalStr = (char*)malloc( sizeof(char)*(SHA512_DIGEST_LENGTH * 2 + 1));
    memset(finalStr,0,SHA512_DIGEST_LENGTH * 2 + 1);
    ...
    memcpy( st_mac, stmac, 6 );
    sprintf(idString,"%02X:%02X:%02X:%02X:%02X:%02X",st_mac[0],st_mac[1],st_mac[2],st_mac[3],st_mac[4],st_mac[5]);

    sha_encryption(idString,outputStr,finalStr);
    //fprintf(stdout,"Mac Id is %s\n",finalStr);

    int ret;
    //fprintf(stdout,"Going in DB\n");
      
    gettimeofday(&start, NULL);

    ret=fetch_data(finalStr);
    //fprintf(stdout,"Coming out of DB\n");

    gettimeofday(&stop, NULL);
    
    //fprintf(stdout,"Time taken by db to execute =  %lu \n", stop.tv_usec - start.tv_usec );
    ...
    if(ret == 1)
    { 
        //fprintf(stdout,"Skipping Station\n");
        goto skip_station;
    }*/ 

    //fprintf(stdout,"Not Skipping Station\n");
    ...
    if( st_cur == NULL )
    {
        ...
        st_cur->vendor_index = 0;
        //fprintf(stdout,"station mac by vendor is %02X%02X%02X%02X%02X%02X",st_cur->stmac[0],st_cur->stmac[1],st_cur->stmac[2],st_cur->stmac[3],st_cur->stmac[4],st_cur->stmac[5]);
        memcpy(st_cur->vendor[st_cur->vendor_index],st_cur->stmac,3);
        st_cur->vendor_index++;
	...
	if(ri->ri_channel > 0 && ri->ri_channel <= HIGHEST_CHANNEL)
        ...
        st_cur->lastseq = seq;
        //if packet comes from the client either type from station MAC create a new node.
        if (ri->ri_power > 0)
        {
           goto skip_station;
        }
        st_cur->packet_info_node = insert_packet_info_node(st_cur->packet_info_node);
        //Fetching the info of packets , adding to our defined structure.
        st_cur->packet_info_node->timestamp = time(NULL);
        st_cur->packet_info_node->packet_sequence_no = seq;
        st_cur->packet_info_node->packet_type = (char*) malloc(sizeof(char) * 5);
        memset(st_cur->packet_info_node->packet_type, 0, (sizeof(char) * 5));
        sprintf(st_cur->packet_info_node->packet_type, "%02X", h80211[0]);
        st_cur->packet_info_node->signal_strength = ri->ri_power;
        st_cur->packet_info_node->channel = st_cur->channel;
        st_cur->packet_info_node->destination = (char*) malloc(sizeof(char) * (2 * SHA256_DIGEST_LENGTH) + 1);
        memset(st_cur->packet_info_node->destination, 0, (sizeof(char) * (2 * SHA256_DIGEST_LENGTH)) + 1);

        //Making SSID in the packet infpo struct NULL as of now
        st_cur->packet_info_node->ssid=NULL;

        switch( h80211[1] & 3 )
        {
           case 0:
                  memcpy(mac_des_length, h80211 + 4, 6); //Adhoc
                  break;
           case 1:
                  memcpy(mac_des_length, h80211 + 16, 6); //ToDs
                  break;
           case 2:
                  memcpy(mac_des_length, h80211 + 4, 6); //FromDs
                  break;
           case 3:
                  memcpy(mac_des_length, h80211 + 16, 6); //WDS
                  break;
        }
        //Encrpyting the destination Address in packet using sha1 encryption.
        SHA256_Init(&sha256);
        SHA256_Update(&sha256, mac_des_length, strlen(mac_des_length));
        final_mac_des_length = (unsigned char*) malloc((sizeof(char)*SHA256_DIGEST_LENGTH) + 1);
        memset(final_mac_des_length, 0, (sizeof(char)*SHA256_DIGEST_LENGTH) + 1);
        SHA256_Final(final_mac_des_length, &sha256);
        for (loopcounter = 0; loopcounter < SHA256_DIGEST_LENGTH; loopcounter++)
        {
             sprintf(&st_cur->packet_info_node->destination[loopcounter * 2], "%02x",
                 (unsigned char) final_mac_des_length[loopcounter]);
        }
        st_cur->packet_info_node->destination[SHA256_DIGEST_LENGTH *2] = '\0';
    }
...
//                n = ( p[1] > 32 ) ? 32 : p[1];s
                ...
                //Added the code to add ssid field in the packet_info struct for valid probe requests
                st_cur->packet_info_node->ssid=(char*)malloc(sizeof(char)*strlen( st_cur->probes[st_cur->probe_index])+1 );
                memset(st_cur->packet_info_node->ssid,0,strlen(st_cur->probes[st_cur->probe_index]));
                memcpy(st_cur->packet_info_node->ssid,st_cur->probes[st_cur->probe_index],strlen(st_cur->probes[st_cur->probe_index]));
                int len=strlen(st_cur->probes[st_cur->probe_index]);
                st_cur->packet_info_node->ssid[len]='\0';
            ...
            if(p[0] == 0xdd){
            	...
            	if(st_cur->vendor_index < 7){
            		memset(st_cur->vendor[st_cur->vendor_index],0,sizeof(st_cur->vendor[st_cur->vendor_index]));
            		for(loopCntr=0;loopCntr<st_cur->vendor_index;loopCntr++){
            			if(memcmp(st_cur->vendor[loopCntr],p+2,3)==0){
            				loopCntr = -1;
            				break;
            			}
            		}
            		if(loopCntr!=-1){
            			memcpy(st_cur->vendor[st_cur->vendor_index],p+2,4);
            			st_cur->vendor_index+=1;
            		}
            	}
            }
            p += 2 + p[1];
...
/* Doing sha256 encryption  on contents of id_string, output_str, final_str */
void sha_encryption(char *id_string, unsigned char *output_str, char *final_str)
{
    int loopCounter = 0;
    SHA256_CTX sha256;
    //SHA_CTX sha1;

    /* Doing sha256 encryption*/
    SHA256_Init(&sha256);
    SHA256_Update(&sha256, id_string, strlen(id_string));
    SHA256_Final(output_str, &sha256);
    for (loopCounter = 0; loopCounter < SHA256_DIGEST_LENGTH; loopCounter++) {
        sprintf(&final_str[loopCounter * 2], "%02x", (unsigned char) output_str[loopCounter]);
    }
    final_str[SHA256_DIGEST_LENGTH * 2] = '\0';
    /* Doing sha1 encryption
    SHA1_Init(&sha1);
    SHA1_Update(&sha1, final_str, strlen(final_str));
    SHA1_Final(output_str, &sha1);
    for (loopCounter = 0; loopCounter < SHA_DIGEST_LENGTH; loopCounter++) {
        sprintf(&final_str[loopCounter * 2], "%02x", (unsigned char) output_str[loopCounter]);
    }
    final_str[SHA_DIGEST_LENGTH * 2] = '\0'; */
}

/* Freeing the memory allocated in the csv_or_protobuf funtion.*/
void free_protobuf_memory(int ap_count, int st_count, Beacon * beacon)
{
    int i = 0;

    /* Freeing up memory of all fields in all BSSID structures */
    for (i = 0; i < ap_count; i++) {
        if (beacon->bssids[i]->bssid != NULL) {
            free(beacon->bssids[i]->bssid);
            beacon->bssids[i]->bssid = NULL;
        }
        /*if (beacon->bssids[i]->firsttimeseen != NULL) {
            free(beacon->bssids[i]->firsttimeseen);
            beacon->bssids[i]->firsttimeseen = NULL;
        }
        if (beacon->bssids[i]->lasttimeseen != NULL) {
            free(beacon->bssids[i]->lasttimeseen);
            beacon->bssids[i]->lasttimeseen = NULL;
        }
        if (beacon->bssids[i]->privacy != NULL) {
            free(beacon->bssids[i]->privacy);
            beacon->bssids[i]->privacy = NULL;
        }
        if (beacon->bssids[i]->cipher != NULL) {
            free(beacon->bssids[i]->cipher);
            beacon->bssids[i]->cipher = NULL;
        }
        if (beacon->bssids[i]->authenication != NULL) {
            free(beacon->bssids[i]->authenication);
            beacon->bssids[i]->authenication = NULL;
        }
        if (beacon->bssids[i]->lanip != NULL) {
            free(beacon->bssids[i]->lanip);
            beacon->bssids[i]->lanip = NULL;
        }*/
        if (beacon->bssids[i]->essid != NULL) {
            free(beacon->bssids[i]->essid);
            beacon->bssids[i]->essid = NULL;
        }
        /*if (beacon->bssids[i]->key != NULL) {
            free(beacon->bssids[i]->key);
            beacon->bssids[i]->key = NULL;
        }*/
        free(beacon->bssids[i]);
        beacon->bssids[i] = NULL;
    }
    free(beacon->bssids);
    beacon->bssids = NULL;

    /* Freeing up memory of all fields in all DEVICE structures*/
    for (i = 0; i < st_count; i++) {
        int n;
        size_t t;

        if (beacon->devices[i]->mac != NULL) {
            free(beacon->devices[i]->mac);
            beacon->devices[i]->mac = NULL;
        }
        if (beacon->devices[i]->fiveoctets != NULL) {
            free(beacon->devices[i]->fiveoctets);
            beacon->devices[i]->fiveoctets = NULL;
        }

        if (beacon->devices[i]->firsttimeseen != NULL) {
            free(beacon->devices[i]->firsttimeseen);
            beacon->devices[i]->firsttimeseen = NULL;
        }
        if (beacon->devices[i]->lasttimeseen != NULL) {
            free(beacon->devices[i]->lasttimeseen);
            beacon->devices[i]->lasttimeseen = NULL;
        }
        if (beacon->devices[i]->bssid != NULL) {
            free(beacon->devices[i]->bssid);
            beacon->devices[i]->bssid = NULL;
        }
        if (beacon->devices[i]->probedessid != NULL) {
            for (n = 0; n < NB_PRB; n++) {
                if (beacon->devices[i]->probedessid[n] != NULL) {
                    free(beacon->devices[i]->probedessid[n]);
                    beacon->devices[i]->probedessid[n] = NULL;
                }
            }
            free(beacon->devices[i]->probedessid);
            beacon->devices[i]->probedessid = NULL;
        }
        if (beacon->devices[i]->vendorouid != NULL) {
            for (t = 0; t < beacon->devices[i]->n_vendorouid; t++) {
                if (beacon->devices[i]->vendorouid[t] != NULL) {
                    free(beacon->devices[i]->vendorouid[t]);
                    beacon->devices[i]->vendorouid[t] = NULL;
                }
            }
            free(beacon->devices[i]->vendorouid);
            beacon->devices[i]->vendorouid = NULL;
        }
        if (beacon->devices[i]->pkts != NULL) {
            for (t = 0; t < beacon->devices[i]->n_pkts; t++) {
                if (beacon->devices[i]->pkts[t]->timestamp != NULL) {
                    free(beacon->devices[i]->pkts[t]->timestamp);
                    beacon->devices[i]->pkts[t]->timestamp = NULL;
                }

                if (beacon->devices[i]->pkts[t]->ssid != NULL) {
                    free(beacon->devices[i]->pkts[t]->ssid);
                    beacon->devices[i]->pkts[t]->ssid = NULL;
                }

                if (beacon->devices[i]->pkts[t]->pkttype !=NULL) {
                    free(beacon->devices[i]->pkts[t]->pkttype);
                    beacon->devices[i]->pkts[t]->pkttype = NULL;
                }
                if (beacon->devices[i]->pkts[t]->destination != NULL) {
                    free(beacon->devices[i]->pkts[t]->destination);
                    beacon->devices[i]->pkts[t]->destination = NULL;
                }
                free(beacon->devices[i]->pkts[t]);
                beacon->devices[i]->pkts[t] = NULL;
            }
            free(beacon->devices[i]->pkts);
            beacon->devices[i]->pkts = NULL;
        }
        free(beacon->devices[i]);
        beacon->devices[i] = NULL;
    }
    free(beacon->devices);
    beacon->devices = NULL;

    free(beacon->deviceid);
    beacon->deviceid = NULL;
    free(beacon->timestamp);
    beacon->timestamp = NULL;
}

/* This function packs the output of the airmon ng into a PROTOBUF or CSV,
 * depending upon in what format we need the output. For PROTOBUF this function
 * will create a debug.csv file which we cam use for debugging purpose. PROTOBUF
 * fills the Bssid, Device, Beacon structures fields and Beacon structure is
 * packed in PROTOBUF.*/
int dump_write_csv_or_protobuf(enum OutputType outputType,char* originaltime)
{
    int i = 0, n = 0, probes_written = 0;
    ...
    char idString[ID_STRING_LEN] = { 0 };
    unsigned char outputStr[SHA512_DIGEST_LENGTH] = { 0 };
    char finalStr[SHA512_DIGEST_LENGTH * 2 + 1] = { 0 };
    char mac_string[ID_STRING_LEN] = { 0 };
    unsigned char output_str[SHA512_DIGEST_LENGTH] = { 0 };
    char final_str[SHA512_DIGEST_LENGTH * 2 + 1] = { 0 };
    
    char rocksDB_idStr[ID_STRING_LEN] = { 0 };
    unsigned char rocksDB_outputStr[SHA512_DIGEST_LENGTH] = { 0 };
    char rocksDB_finalStr[SHA512_DIGEST_LENGTH * 2 + 1]={0};
    
    int loopCntr = 0;
    SHA_CTX ctx2;
    Beacon beacon = BEACON__INIT;
    int ap_count = 0, ap_id = 0, st_count = 0, st_id = 0;
    struct ST_info *previous = NULL;


    if (OUTPUT_TYPE_CSV == outputType) {
        if (!G.record_data || !G.output_format_csv)
            return 0;
    } else if (OUTPUT_TYPE_PROTOBUF == outputType) {
        struct AP_info *ap_traverse = NULL;
        struct ST_info *st_traverse = NULL;
        Bssid **bssids = NULL;
        Device **devices = NULL;

        if (!G.record_data || !G.output_format_protobuf)
            return 0;

        /* Retaining truncated CSVs in /tmp for debugging */
        if ((G.f_txt = fopen("/tmp/debug.csv", "w")) == NULL) {
            perror("fopen failed");
            fprintf(stderr, "Could not create \"%s\".\n", "/tmp/debug.csv");
            return (1);
        }
        if (ftruncate(fileno(G.f_txt), 0) != 0) {
            perror("ftruncate failed on f_txt");
            fprintf(stderr, "Could not truncate debug CSV file.\n");
            return (1);
        }

        /* First, we find the number of BSSID structures we need */
        ap_traverse = G.ap_1st;
        while (ap_traverse != NULL) {
            ap_count++;
            ap_traverse = ap_traverse->next;
        }

        /* Then we declare those structures */
        bssids = malloc(sizeof(Bssid *) * ap_count);
        for (i = 0; i < ap_count; i++) {
            bssids[i] = malloc(sizeof(Bssid));
            bssid__init(bssids[i]);
        }

        beacon.n_bssids = ap_count;
        beacon.bssids = bssids;

        /* We find the number of DEVICE structures we need */
        st_count=0;
        st_traverse = G.st_1st;
       
        
        while (st_traverse != NULL) {
            

            memset(rocksDB_finalStr,0,SHA512_DIGEST_LENGTH * 2 + 1);

            sprintf(rocksDB_idStr,"%02X:%02X:%02X:%02X:%02X:%02X",st_traverse->stmac[0], st_traverse->stmac[1],st_traverse->stmac[2],
                st_traverse->stmac[3], st_traverse->stmac[4], st_traverse->stmac[5]);
     
            rocksDB_idStr[0] = '\0';
            rocksDB_outputStr[0] = '\0';
            sha_encryption(rocksDB_idStr,rocksDB_outputStr,rocksDB_finalStr);

            int ret;
            ret=check_rocksDB(rocksDB_finalStr);
            ...
            st_traverse = st_traverse->next;
        
        }
       
    //    fprintf(stdout,"value of st_count is %d", st_count);
        /*Declaration of those DEVICE structures */
        devices = malloc(sizeof(Device *) * st_count);
        for (i = 0; i < st_count; i++) {
            devices[i] = malloc(sizeof(Device));
            device__init(devices[i]);
        }

        beacon.n_devices = st_count;
        beacon.devices = devices;
    }
    ...
    if (OUTPUT_TYPE_PROTOBUF == outputType) {
        FILE *fp_device_id = NULL;
        time_t now = time(NULL);

        fp_device_id = fopen("/etc/adx_scratchpad/device_id.txt", "r");
        beacon.deviceid = (char *) malloc(MAXIMUM_DEVICE_ID_LENGTH + 1);
        memset(beacon.deviceid, 0, MAXIMUM_DEVICE_ID_LENGTH + 1);
        if ((fread(beacon.deviceid, MAXIMUM_DEVICE_ID_LENGTH + 1, 1, fp_device_id)))
            strncpy(beacon.deviceid, DEVICE_ID_NOT_FOUND, MAXIMUM_DEVICE_ID_LENGTH);

        beacon.has_formatversion = 1;
        beacon.formatversion = 2;

        //ltime = localtime(&now);
        beacon.timestamp = (char *) malloc(MAXIMUM_TIMESTAMP_LENGTH + 1);
        memset(beacon.timestamp, 0, MAXIMUM_TIMESTAMP_LENGTH + 1);
       //strftime(beacon.timestamp, MAXIMUM_TIMESTAMP_LENGTH, "%FT%TZ", ltime);
        sprintf(beacon.timestamp,"%s",originaltime);
    ...
    while (ap_cur != NULL) {
        char print_buffer[MAXIMUM_BUFFER_LENGTH] = { 0 };
        ...
        sprintf(idString, "%02X:%02X:%02X:%02X:%02X:%02X:%02X",
                ap_cur->bssid[0], ap_cur->bssid[1], ap_cur->bssid[2],
                ap_cur->power_bit, ap_cur->bssid[3], ap_cur->bssid[4], ap_cur->bssid[5]);
        sha_encryption(idString, outputStr, finalStr);
        fprintf(G.f_txt, "%s, ", finalStr);
        if (OUTPUT_TYPE_PROTOBUF == outputType) {
            beacon.bssids[ap_id]->bssid = (char *) malloc(strlen(finalStr) + 1);
            memset(beacon.bssids[ap_id]->bssid, 0, (strlen(finalStr) + 1));
            memcpy(beacon.bssids[ap_id]->bssid, finalStr, strlen(finalStr));
        }
        ...
        /*if (OUTPUT_TYPE_PROTOBUF == outputType) {
            beacon.bssids[ap_id]->firsttimeseen = (char *) malloc((MAXIMUM_TIMESTAMP_LENGTH) + 1);
            memset(beacon.bssids[ap_id]->firsttimeseen, 0, (MAXIMUM_TIMESTAMP_LENGTH) + 1);
            strftime(beacon.bssids[ap_id]->firsttimeseen,
                     MAXIMUM_TIMESTAMP_LENGTH, "%FT%TZ", ltime);
        }*/
        ...
        fprintf(G.f_txt, "%04d-%02d-%02d %02d:%02d:%02d, ",
                ...
                ltime->tm_mday, ltime->tm_hour, ltime->tm_min, ltime->tm_sec);
        /*if (OUTPUT_TYPE_PROTOBUF == outputType) {
            beacon.bssids[ap_id]->lasttimeseen = (char *) malloc((MAXIMUM_TIMESTAMP_LENGTH) + 1);
            memset(beacon.bssids[ap_id]->lasttimeseen, 0, (MAXIMUM_TIMESTAMP_LENGTH) + 1);
            strftime(beacon.bssids[ap_id]->lasttimeseen, MAXIMUM_TIMESTAMP_LENGTH, "%FT%TZ", ltime);
        }*/

        fprintf(G.f_txt, "%2d, %3d,", ap_cur->channel, ap_cur->max_speed);
        /*if (OUTPUT_TYPE_PROTOBUF == outputType) {
            beacon.bssids[ap_id]->has_channel = 1;
            beacon.bssids[ap_id]->has_speed = 1;
            beacon.bssids[ap_id]->channel = ap_cur->channel;
            beacon.bssids[ap_id]->speed = ap_cur->max_speed;
        }*/

        memset(print_buffer, 0, sizeof(print_buffer));
        print_buffer_length = 0;
        if ((ap_cur->security & (STD_OPN | STD_WEP | STD_WPA | STD_WPA2)) == 0)
            print_buffer_length += sprintf(print_buffer, " ");
        else {
            if (ap_cur->security & STD_WPA2)
                print_buffer_length += sprintf(print_buffer + print_buffer_length, "WPA2 ");
            if (ap_cur->security & STD_WPA)
                print_buffer_length += sprintf(print_buffer + print_buffer_length, "WPA ");
            if (ap_cur->security & STD_WEP)
                print_buffer_length += sprintf(print_buffer + print_buffer_length, "WEP ");
            if (ap_cur->security & STD_OPN)
                print_buffer_length += sprintf(print_buffer + print_buffer_length, "OPN ");
        }
        fprintf(G.f_txt, "%s", print_buffer);

        /*if (OUTPUT_TYPE_PROTOBUF == outputType) {
            beacon.bssids[ap_id]->privacy = (char *) malloc(strlen(print_buffer) + 1);
            memset(beacon.bssids[ap_id]->privacy, 0, (strlen(print_buffer) + 1));
            memcpy(beacon.bssids[ap_id]->privacy, print_buffer, strlen(print_buffer));
        }*/
        ...
        print_buffer_length = 0;
        if ((ap_cur->
             security & (ENC_WEP | ENC_TKIP | ENC_WRAP | ENC_CCMP | ENC_WEP104 | ENC_WEP40)) == 0)
            print_buffer_length += sprintf(print_buffer, " ");
        else {
            if (ap_cur->security & ENC_CCMP)
                print_buffer_length += sprintf(print_buffer + print_buffer_length, "CCMP ");
            if (ap_cur->security & ENC_WRAP)
                print_buffer_length += sprintf(print_buffer + print_buffer_length, "WRAP ");
            if (ap_cur->security & ENC_TKIP)
                print_buffer_length += sprintf(print_buffer + print_buffer_length, "TKIP ");
            if (ap_cur->security & ENC_WEP104)
                print_buffer_length += sprintf(print_buffer + print_buffer_length, "WEP104 ");
            if (ap_cur->security & ENC_WEP40)
                print_buffer_length += sprintf(print_buffer + print_buffer_length, "WEP40 ");
        ...
        /*if (OUTPUT_TYPE_PROTOBUF == outputType) {
            ...
            beacon.bssids[ap_id]->cipher = (char *) malloc(strlen(print_buffer) + 1);
            memset(beacon.bssids[ap_id]->cipher, 0, (strlen(print_buffer) + 1));
            memcpy(beacon.bssids[ap_id]->cipher, print_buffer, strlen(print_buffer));
        }*/
        ...
        if ((ap_cur->security & (AUTH_OPN | AUTH_PSK | AUTH_MGT)) == 0)
            ...
            if (ap_cur->security & AUTH_MGT)
                print_buffer_length += sprintf(print_buffer + print_buffer_length, "MGT ");
            if (ap_cur->security & AUTH_PSK) {
                if (ap_cur->security & STD_WEP)
                    print_buffer_length += sprintf(print_buffer + print_buffer_length, "SKA ");
                else
                    print_buffer_length += sprintf(print_buffer + print_buffer_length, "PSK ");
            }
                print_buffer_length += sprintf(print_buffer + print_buffer_length, "OPN ");
        ...
        fprintf(G.f_txt, "%s", print_buffer);
        /*if (OUTPUT_TYPE_PROTOBUF == outputType) {
            beacon.bssids[ap_id]->authenication = (char *) malloc((strlen(print_buffer) + 1));
            memset(beacon.bssids[ap_id]->authenication, 0, (strlen(print_buffer) + 1));
            memcpy(beacon.bssids[ap_id]->authenication, print_buffer, strlen(print_buffer));
        }*/

        fprintf(G.f_txt, ", %3d, %8ld, %8ld, ", ap_cur->avg_power, ap_cur->nb_bcn, ap_cur->nb_data);
        /*if (OUTPUT_TYPE_PROTOBUF == outputType) {
            beacon.bssids[ap_id]->has_power = 1;
            beacon.bssids[ap_id]->has_nobeacons = 1;
            beacon.bssids[ap_id]->has_noiv = 1;
            beacon.bssids[ap_id]->power = ap_cur->avg_power;
            beacon.bssids[ap_id]->nobeacons = ap_cur->nb_bcn;
            beacon.bssids[ap_id]->noiv = ap_cur->nb_data;
        }*/
        fprintf(G.f_txt, "%3d.%3d.%3d.%3d, ",
                ap_cur->lanip[0], ap_cur->lanip[1], ap_cur->lanip[2], ap_cur->lanip[3]);
        memset(print_buffer, 0, sizeof(print_buffer));
        /*if (OUTPUT_TYPE_PROTOBUF == outputType) {
            sprintf(print_buffer, "%3d.%3d.%3d.%3d, ", ap_cur->lanip[0], ap_cur->lanip[1],
                    ap_cur->lanip[2], ap_cur->lanip[3]);
            beacon.bssids[ap_id]->lanip = (char *) malloc(strlen(print_buffer) + 1);
            memset(beacon.bssids[ap_id]->lanip, 0, (strlen(print_buffer) + 1));
            memcpy(beacon.bssids[ap_id]->lanip, print_buffer, strlen(print_buffer));
        }*/

        fprintf(G.f_txt, "%3d, ", ap_cur->ssid_length);
        /*if (OUTPUT_TYPE_PROTOBUF == outputType) {
            beacon.bssids[ap_id]->has_idlen = 1;
            beacon.bssids[ap_id]->idlen = ap_cur->ssid_length;
        }*/

        temp = format_text_for_csv_or_protobuf(ap_cur->essid, ap_cur->ssid_length);
        fprintf(G.f_txt, "%s, ", temp);
        if (OUTPUT_TYPE_PROTOBUF == outputType) {
            beacon.bssids[ap_id]->essid = (char *) malloc((strlen(temp)) + 1);
            memset(beacon.bssids[ap_id]->essid, 0, (strlen(temp) + 1));
            memcpy(beacon.bssids[ap_id]->essid, temp, strlen(temp));
        }
        free(temp);
        ...
        /*if (OUTPUT_TYPE_PROTOBUF == outputType) {
            ...
            if (ap_cur->key != NULL) {
                int n = (int) strlen(ap_cur->key);
                key_buffer = (char *) malloc(sizeof(char) * (n * 3));
                for (i = 0; i < (int) strlen(ap_cur->key); i++) {
                    sprintf(&key_buffer[i * 3], "%02X", ap_cur->key[i]);
                    if (i < (int) (strlen(ap_cur->key) - 1)) {
                        sprintf(&key_buffer[(i * 3) + 2], ":");
                    }
                }
            }
            beacon.bssids[ap_id]->key = key_buffer;
        }*/
        fprintf(G.f_txt, "\r\n");
    ...
    while (st_cur != NULL) {
        ...
        idString[0] = '\0';
        outputStr[0] = '\0';
        sprintf(idString, "%02X:%02X:%02X:%02X:%02X:%02X",
                st_cur->stmac[0], st_cur->stmac[1], st_cur->stmac[2],
                st_cur->stmac[3], st_cur->stmac[4], st_cur->stmac[5]);
        sha_encryption(idString, outputStr, finalStr);
        fprintf(G.f_txt, "%s, ", finalStr);
        
         /* Checking if the Station MAC is to be packed or not according to the DB specification */
        int ret_val;
        ret_val=check_rocksDB(finalStr);
        
       
        if (ret_val != 1)
        {

            //Getting first five octets of station MAC
            mac_string[0] = '\0';
            output_str[0] = '\0';
            sprintf(mac_string,"%02X:%02X:%02X:%02X:%02X",st_cur->stmac[0],st_cur->stmac[1],st_cur->stmac[2],st_cur->stmac[3],st_cur->stmac[4]);
            //Hashing first five Octets of station MAC
            sha_encryption(mac_string,output_str,final_str);
            
            
            if (OUTPUT_TYPE_PROTOBUF == outputType) {
                beacon.devices[st_id]->mac = (char *) malloc(strlen(finalStr) + 1);
                memset(beacon.devices[st_id]->mac, 0, (strlen(finalStr) + 1));
                memcpy(beacon.devices[st_id]->mac, finalStr, strlen(finalStr));
                //fprintf(stdout,"Station MAC is- %s\n",beacon.devices[st_id]->mac);      


                //Packing it to beacon structure defined in lrm.proto
                beacon.devices[st_id]->fiveoctets = (char *) malloc(strlen(final_str) + 1);
                memset(beacon.devices[st_id]->fiveoctets, 0, (strlen(final_str) + 1));
                memcpy(beacon.devices[st_id]->fiveoctets, final_str, strlen(final_str));
                beacon.devices[st_id]->fiveoctets[strlen(final_str)]='\0';
            }
            ...
            fprintf(G.f_txt, "%04d-%02d-%02d %02d:%02d:%02d, ",
                    ...
                    ltime->tm_mday, ltime->tm_hour, ltime->tm_min, ltime->tm_sec);
            if (OUTPUT_TYPE_PROTOBUF == outputType) {
                beacon.devices[st_id]->firsttimeseen = (char *) malloc((MAXIMUM_TIMESTAMP_LENGTH) + 1);
                memset(beacon.devices[st_id]->firsttimeseen, 0, (MAXIMUM_TIMESTAMP_LENGTH) + 1);
                strftime(beacon.devices[st_id]->firsttimeseen, MAXIMUM_TIMESTAMP_LENGTH, "%FT%TZ",
                            ltime);
            }
            ...
            fprintf(G.f_txt, "%04d-%02d-%02d %02d:%02d:%02d, ",
                    ...
                    ltime->tm_mday, ltime->tm_hour, ltime->tm_min, ltime->tm_sec);
            if (OUTPUT_TYPE_PROTOBUF == outputType) {
                beacon.devices[st_id]->lasttimeseen = (char *) malloc((MAXIMUM_TIMESTAMP_LENGTH) + 1);
                memset(beacon.devices[st_id]->lasttimeseen, 0, (MAXIMUM_TIMESTAMP_LENGTH) + 1);
                strftime(beacon.devices[st_id]->lasttimeseen, MAXIMUM_TIMESTAMP_LENGTH, "%FT%TZ",
                            ltime);
            }

            fprintf(G.f_txt, "%3d, %8ld, ", st_cur->power, st_cur->nb_pkt);
            if (OUTPUT_TYPE_PROTOBUF == outputType) {
                beacon.devices[st_id]->has_power = 1;
                beacon.devices[st_id]->has_nopackets = 1;
                beacon.devices[st_id]->power = st_cur->power;
                beacon.devices[st_id]->nopackets = st_cur->nb_pkt;
            }

            memset(print_buffer, 0, sizeof(print_buffer));
            if (!memcmp(ap_cur->bssid, BROADCAST, 6)) {
                sprintf(print_buffer, "not associated");
                fprintf(G.f_txt, "%s", print_buffer);
                if (OUTPUT_TYPE_PROTOBUF == outputType) {
                    beacon.devices[st_id]->bssid = (char *) malloc((strlen(print_buffer)) + 1);
                    memset(beacon.devices[st_id]->bssid, 0, (strlen(print_buffer) + 1));
                    memcpy(beacon.devices[st_id]->bssid, print_buffer, strlen(print_buffer));
                }
            } else {
                idString[0] = '\0';
                outputStr[0] = '\0';
                ...
                fprintf(G.f_txt, "%s, ", finalStr);
                if (OUTPUT_TYPE_PROTOBUF == outputType) {
                    beacon.devices[st_id]->bssid = (char *) malloc(strlen(finalStr) + 1);
                    memset(beacon.devices[st_id]->bssid, 0, (strlen(finalStr) + 1));
                    memcpy(beacon.devices[st_id]->bssid, finalStr, strlen(finalStr));
                }
            }

            if (OUTPUT_TYPE_PROTOBUF == outputType) {
                beacon.devices[st_id]->probedessid = (char **) malloc(sizeof(char *) * NB_PRB);
            }
            probes_written = 0;
            for (i = 0, n = 0; i < NB_PRB; i++) {
                if (OUTPUT_TYPE_PROTOBUF == outputType) {
                    beacon.devices[st_id]->probedessid[i] = NULL;
                }
                if (st_cur->ssid_length[i] == 0) {
                    continue;
                }
                temp = format_text_for_csv_or_protobuf((unsigned char *) st_cur->probes[i],
                                                        st_cur->ssid_length[i]);

                memset(outputStr, 0, sizeof(outputStr) - 1);
                memset(finalStr, 0, sizeof(finalStr) - 1);
                SHA1_Init(&ctx2);
                SHA1_Update(&ctx2, temp, strlen(temp));
                SHA1_Final(outputStr, &ctx2);
                ...
                for (loopCntr = 0; loopCntr < SHA_DIGEST_LENGTH; loopCntr++) {
                    sprintf(&finalStr[loopCntr * 2], "%02x", (unsigned char) outputStr[loopCntr]);
                }
                fprintf(G.f_txt, "%s", finalStr);
                if (OUTPUT_TYPE_PROTOBUF == outputType) {

                    beacon.devices[st_id]->probedessid[n] = (char *) malloc(strlen(finalStr) + 1);
                    memset(beacon.devices[st_id]->probedessid[n], 0, (strlen(finalStr) + 1));
                    memcpy(beacon.devices[st_id]->probedessid[n], finalStr, strlen(finalStr));
                    n++;
                }
                free(temp);
            }
            if (OUTPUT_TYPE_PROTOBUF == outputType) {
                beacon.devices[st_id]->n_probedessid = n;
            }

            fprintf(G.f_txt, ",||,");
            if (OUTPUT_TYPE_PROTOBUF == outputType) {
                if (st_cur->vendor_index != 0) {
                    beacon.devices[st_id]->vendorouid =
                        (char **) malloc(sizeof(char *) * (st_cur->vendor_index));
                    memset(beacon.devices[st_id]->vendorouid, 0, ((sizeof(char *) * (st_cur->vendor_index))));
                }
                beacon.devices[st_id]->n_vendorouid = st_cur->vendor_index;
            }
            for (loopCntr = 0; loopCntr < st_cur->vendor_index; loopCntr++) {
                if (OUTPUT_TYPE_PROTOBUF == outputType) {
                    beacon.devices[st_id]->vendorouid[loopCntr] = NULL;
                }
                memset(finalStr, 0, sizeof(finalStr) - 1);
                for (i = 0; i < 3; i++) {
                    sprintf(&finalStr[i * 2], "%02x", (unsigned char) st_cur->vendor[loopCntr][i]);
                }
                fprintf(G.f_txt, "%s,", finalStr);
                if (OUTPUT_TYPE_PROTOBUF == outputType) {
                    beacon.devices[st_id]->vendorouid[loopCntr] = (char *) malloc(strlen(finalStr) + 1);
                    memset(beacon.devices[st_id]->vendorouid[loopCntr], 0, (strlen(finalStr) + 1));
                    memcpy(beacon.devices[st_id]->vendorouid[loopCntr], finalStr, strlen(finalStr));
                }
            }


            if (OUTPUT_TYPE_PROTOBUF == outputType) {
                struct packet_info *temp_node = NULL;
                int no_of_packets = 0;
                temp_node = st_cur->packet_info_node;

                beacon.devices[st_id]->has_devicetype = 1;
                beacon.devices[st_id]->devicetype = 0;

                while (temp_node != NULL) {
                    //Condition to discard the station MAC  which are transmitting packet types of 88,80 or 50 and iterate the loop to next station MAC
                    if((strncmp(temp_node->packet_type,"88",2) == 0) || (strncmp(temp_node->packet_type,"50",2) == 0) || (strncmp(temp_node->packet_type,"80",2) == 0))
                    {
                        beacon.devices[st_id]->has_devicetype = 1;
                        beacon.devices[st_id]->devicetype = 1;
                        break;
                    }
                    no_of_packets++;
                    temp_node = temp_node->next;
                }
                //If discarding condition occurs, iterate the loop to next station device
                if(temp_node != NULL) {
                    previous = st_cur;
                    st_cur = st_cur->next;
                    st_id++;
                ...
                beacon.devices[st_id]->n_pkts = no_of_packets;
                beacon.devices[st_id]->pkts = (Packet** ) malloc(sizeof(Packet*) * (no_of_packets));
                temp_node = st_cur->packet_info_node;

                i = 0;
                while (temp_node != NULL) {
                    beacon.devices[st_id]->pkts[i] = (Packet*) malloc(sizeof(Packet));
                    packet__init(beacon.devices[st_id]->pkts[i]);

                    ltime = localtime(&temp_node->timestamp);
                    beacon.devices[st_id]->pkts[i]->timestamp = (char*) malloc(MAXIMUM_TIMESTAMP_LENGTH + 1);
                    memset(beacon.devices[st_id]->pkts[i]->timestamp, 0, (MAXIMUM_TIMESTAMP_LENGTH + 1));
                    strftime(beacon.devices[st_id]->pkts[i]->timestamp, MAXIMUM_TIMESTAMP_LENGTH, "%FT%TZ", ltime);

                    beacon.devices[st_id]->pkts[i]->has_seqnum = 1;
                    beacon.devices[st_id]->pkts[i]->seqnum = temp_node->packet_sequence_no;

                    beacon.devices[st_id]->pkts[i]->pkttype = (char*) malloc(strlen(temp_node->packet_type) + 1);
                    memset(beacon.devices[st_id]->pkts[i]->pkttype, 0, (strlen(temp_node->packet_type) + 1));
                    memcpy(beacon.devices[st_id]->pkts[i]->pkttype, temp_node->packet_type, (strlen(temp_node->packet_type)));


                    //Added the code to add new SSID field in packet struct inside the Beacon for valid probe requests
                    if( strncmp(beacon.devices[st_id]->pkts[i]->pkttype,"40",2) == 0 && temp_node->ssid!=NULL )
                    {

                        temp = format_text_for_csv_or_protobuf((unsigned char *) temp_node->ssid,
                                                        strlen(temp_node->ssid));
                        ...
                        beacon.devices[st_id]->pkts[i]->ssid=(char*)malloc(sizeof(char)*strlen(finalStr)+1 );
                        memset(beacon.devices[st_id]->pkts[i]->ssid,0,strlen(finalStr));
                        memcpy(beacon.devices[st_id]->pkts[i]->ssid, finalStr, strlen(finalStr));
                        beacon.devices[st_id]->pkts[i]->ssid[strlen(finalStr)]='\0';

                        free(temp);
                        temp='\0';
                    }

                    beacon.devices[st_id]->pkts[i]->has_rssi = 1;
                    beacon.devices[st_id]->pkts[i]->rssi = temp_node->signal_strength;

                    beacon.devices[st_id]->pkts[i]->destination = (char*) malloc(strlen(temp_node->destination) + 1);
                    memset(beacon.devices[st_id]->pkts[i]->destination, 0, (strlen(temp_node->destination) + 1));
                    memcpy(beacon.devices[st_id]->pkts[i]->destination, temp_node->destination, strlen(temp_node->destination));

                    beacon.devices[st_id]->pkts[i]->has_channel = 1;
                    beacon.devices[st_id]->pkts[i]->channel = temp_node->channel;
    ...
    if (OUTPUT_TYPE_PROTOBUF == outputType) {
        unsigned packed_buffer_len = beacon__get_packed_size(&beacon);
        void *packed_buffer = malloc(packed_buffer_len);
        beacon__pack(&beacon, packed_buffer);

        if (ftruncate(fileno(G.f_protobuf), 0) != 0) {
            perror("ftruncate failed on f_protobuf");
            fprintf(stderr, "Could not truncate protobuf file.\n");
            return (1);
        }
        if (fseek(G.f_protobuf, 0, SEEK_SET) != 0) {
            perror("fseek failed on f_protobuf");
            fprintf(stderr, "Could not fseek protobuf file.\n");
            return (1);
        }
        fwrite(packed_buffer, packed_buffer_len, 1, G.f_protobuf);
        fflush(G.f_protobuf);

        free(packed_buffer);
        free_protobuf_memory(ap_count, st_count, &beacon);
    }
				...
				while (output_format_string != NULL) {
					if (strlen(output_format_string) != 0) {
						if (strncasecmp(output_format_string, "csv", 3) == 0
							...
							G.output_format_csv = 1;
						} else if (strncasecmp(output_format_string, "protobuf", 8) == 0) {
							G.output_format_protobuf = 1;
...
//             printf("finished rearranging\n");
    ...
    open_rocksDB();
    char* originaltime = NULL;
    originaltime = (char*) malloc (MAXIMUM_TIMESTAMP_LENGTH + 1);
    memset(originaltime, 0, MAXIMUM_TIMESTAMP_LENGTH + 1);
    time_t loctime = time(NULL);
    struct tm *now = NULL;
    now = localtime(&loctime);
    strftime(originaltime, MAXIMUM_TIMESTAMP_LENGTH , "%FT%TZ", now);
    ...
    while( 1 )
    {
        ...
        if( time( NULL ) - tt1 >= G.file_write_interval  )
        {
            ...
            if (G. output_format_csv)  dump_write_csv_or_protobuf(OUTPUT_TYPE_CSV,originaltime);
            if (G.output_format_protobuf) dump_write_csv_or_protobuf(OUTPUT_TYPE_PROTOBUF,originaltime);
...
//                         return 1;
    ...
    if (G.record_data) {
        ...
        if ( G.output_format_protobuf || G.f_protobuf != NULL ) fclose( G.f_protobuf );
    ...
    while(st_cur != NULL)
    {
        while (st_cur->packet_info_node) {
            temp_node =  st_cur->packet_info_node;
            st_cur->packet_info_node = st_cur->packet_info_node->next;
            free(temp_node);
            temp_node = NULL;
        }
        st_next = st_cur->next;
